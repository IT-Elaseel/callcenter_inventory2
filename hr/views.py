...
def applicant_create(request):
    ...
    # ✅ جروبات الفيلدز عشان نستخدمها في الـ template بدل split
    fields_basic = [
...
def accepted_list(request):
    ...
    # 🟢 لو مفيش قيم من الفورم خليهم بتاريخ النهارده
    if not date_from:
    ...
    # ✅ لو from أكبر من to خليهم متساويين
    if dt_from > dt_to:
        dt_from = dt_to
        date_from = date_to
...
def _get_next_pending(current_order_number=None):
    ...
    if current_order_number:
        return qs.filter(order_number__gt=int(current_order_number)).first()
    return qs.first()


def _get_prev_pending(current_order_number=None):
    qs = Applicant.objects.filter(status="pending").order_by("order_number")
    if current_order_number:
        return qs.filter(order_number__lt=int(current_order_number)).last()
    return qs.last()
...
def queue_view(request):
    ...
    return render(request, "hr/queue.html", {
        ...
        "can_next": is_admin_or_hr(request.user),  # زرار التالي يظهر لـ HR فقط
    })
#-------------------------------------------------------------------------------------------------------
@login_required
@user_passes_test(is_admin_or_hr)
def queue_next(request):
    ...
    if next_app:
        ...
        # مفيش بعده → خليه واقف عند الحالي
        messages.info(request, "وصلت لآخر طلب قيد الانتظار.")
    queue.save()
    return redirect("hr:queue_view")

@login_required
@user_passes_test(is_admin_or_hr)
def queue_prev(request):
    queue, _ = Queue.objects.get_or_create(id=1)
    current = queue.current_applicant
    prev_app = _get_prev_pending(current.order_number if current else None)
    queue.current_applicant = prev_app
    queue.save()
    if prev_app:
        messages.success(request, f"السابق: طلب #{prev_app.order_number} - {prev_app.full_name}")
    else:
        messages.info(request, "لا يوجد طلبات سابقة.")
    return redirect("hr:queue_view")
...
# افتراضًا إن عندك الدوال/الموديلات دي
# from .utils import is_hr, is_hr_help
# from .models import Applicant

# ====== مساعد للتنسيق العام للجدول ======
def _style_table(ws):
    # Freeze أول صف
    ws.freeze_panes = "A2"

    # تنسيق الهيدر: خلفية سوداء + خط أبيض Bold
...
def export_applicant_excel(request, order_number):
    ...
    # صلاحيات الاطلاع: HR_HELP يشوف اللي عمله بس
    if is_hr_help(request.user) and not is_hr(request.user):
        if applicant.created_by_id != request.user.id:
    ...
    rows = [
        ...
        ("الحالة الاجتماعية", applicant.get_marital_status_display() if getattr(applicant, "marital_status", None) else ""),
        ("الجنسية", applicant.get_nationality_display() if getattr(applicant, "nationality", None) else ""),
        ("النوع", applicant.get_gender_display() if getattr(applicant, "gender", None) else ""),
        ("الديانة", applicant.get_religion_display() if getattr(applicant, "religion", None) else ""),
        ("الموقف من التجنيد", applicant.get_military_status_display() if getattr(applicant, "military_status", None) else ""),
        ("البريد", applicant.email or ""),
        ("اسم القريب", applicant.relative_name or ""),
        ("هاتف القريب", applicant.relative_phone or ""),
        ("مدخن", "نعم" if applicant.is_smoker else "لا"),
        ("وسيلة/سيارة", applicant.get_vehicle_ownership_display() if getattr(applicant, "vehicle_ownership", None) else ""),
        ("المؤهل", applicant.get_edu_degree_display() if getattr(applicant, "edu_degree", None) else ""),
        ...
        ("التقدير", applicant.get_edu_grade_display() if getattr(applicant, "edu_grade", None) else ""),
        ("الوظيفة المتقدم لها", applicant.get_job_applied_display() if getattr(applicant, "job_applied", None) else ""),
        ("كود الوظيفة", applicant.job_code or ""),
        ("تاريخ التقديم", applicant.submitted_at.strftime("%Y-%m-%d %H:%M:%S") if applicant.submitted_at else ""),
        ...
        ("الحالة الحالية", applicant.get_status_display() if getattr(applicant, "status", None) else ""),
        ("أُنشئ بواسطة", applicant.created_by.get_full_name() if applicant.created_by else ""),
        ("تاريخ الإنشاء", applicant.created_at.strftime("%Y-%m-%d %H:%M:%S") if applicant.created_at else ""),
        ("آخر تعديل بواسطة", applicant.last_updated_by.get_full_name() if applicant.last_updated_by else ""),
        ("آخر تعديل", applicant.updated_at.strftime("%Y-%m-%d %H:%M:%S") if applicant.updated_at else ""),
    ...
    for exp in applicant.experiences.all():
        ws2.append([exp.employer, exp.job_title, exp.years, float(exp.salary) if exp.salary is not None else "", exp.reason_for_leaving])
...
def export_applicants_excel(request):
    ...
    # فلاتر اختيارية: تاريخ من/إلى (على created_at)
    date_from = request.GET.get("from")
    date_to = request.GET.get("to")
    if date_from:
        try:
    ...
    wb = openpyxl.Workbook()
    ws = wb.active
    ws.title = "الطلبات (كامل)"

    # نفس الحقول الموجودة في التصدير الفردي لكن كأعمدة
    headers = [
        "رقم الطلب","الاسم","الرقم القومي","الهاتف",
        "الحالة الاجتماعية","الجنسية","النوع","الديانة","الموقف من التجنيد",
        "البريد","اسم القريب","هاتف القريب","مدخن","وسيلة/سيارة",
        "المؤهل","سنة التخرج","جهة الحصول","التخصص","دراسات عليا","التقدير",
        "الوظيفة المتقدم لها","كود الوظيفة","تاريخ التقديم",
        "سبق التقديم بالشركة","أقارب بالشركة","تفاصيل أقارب بالشركة",
        "أقارب/أصدقاء بشركات منافسة","تفاصيل أقارب بشركات منافسة",
        "مشاكل صحية","تفاصيل المشاكل الصحية",
        "الحالة الحالية","أُنشئ بواسطة","تاريخ الإنشاء",
        "آخر تعديل بواسطة","آخر تعديل","القرار بواسطة","تاريخ القرار"
    ]
    ws.append(headers)
    ...
    for a in qs:
        ws.append([
            ...
            a.national_id,
            a.phone,
            (a.get_marital_status_display() if getattr(a, "marital_status", None) else ""),
            (a.get_nationality_display() if getattr(a, "nationality", None) else ""),
            (a.get_gender_display() if getattr(a, "gender", None) else ""),
            (a.get_religion_display() if getattr(a, "religion", None) else ""),
            (a.get_military_status_display() if getattr(a, "military_status", None) else ""),
            a.email or "",
            a.relative_name or "",
            a.relative_phone or "",
            ("نعم" if a.is_smoker else "لا"),
            (a.get_vehicle_ownership_display() if getattr(a, "vehicle_ownership", None) else ""),
            (a.get_edu_degree_display() if getattr(a, "edu_degree", None) else ""),
            a.grad_year or "",
            a.edu_institution or "",
            a.specialization or "",
            a.postgrad_study or "",
            (a.get_edu_grade_display() if getattr(a, "edu_grade", None) else ""),
            (a.get_job_applied_display() if getattr(a, "job_applied", None) else ""),
            a.job_code or "",
            a.submitted_at.strftime("%Y-%m-%d %H:%M:%S") if a.submitted_at else "",
            ("نعم" if a.prev_applied else "لا"),
            ("نعم" if a.has_relatives_in_company else "لا"),
            a.relatives_in_company or "",
            ("نعم" if a.has_relatives_in_competitors else "لا"),
            a.relatives_in_competitors or "",
            ("نعم" if a.has_health_issues else "لا"),
            a.health_issues_details or "",
            (a.get_status_display() if getattr(a, "status", None) else ""),
            (a.created_by.get_full_name() if a.created_by else ""),
            a.created_at.strftime("%Y-%m-%d %H:%M:%S") if a.created_at else "",
            (a.last_updated_by.get_full_name() if a.last_updated_by else ""),
            a.updated_at.strftime("%Y-%m-%d %H:%M:%S") if a.updated_at else "",
            (a.decision_by.get_full_name() if a.decision_by else ""),
            a.decision_at.strftime("%Y-%m-%d %H:%M:%S") if a.decision_at else "",
        ])

    _style_table(ws)
    _worksheet_autofit(ws)

    # شيت مجمّع للخبرات لكل المتقدمين
    ws2 = wb.create_sheet("الخبرات")
    ws2.append(["رقم الطلب","الاسم","جهة العمل","الوظيفة","السنوات","الراتب","سبب ترك العمل"])
    for a in qs:
        for exp in a.experiences.all():
            ws2.append([
                a.order_number,
                a.full_name,
                exp.employer,
                exp.job_title,
                exp.years,
                float(exp.salary) if exp.salary is not None else "",
