...
from .models import Category, Inventory, Product, Branch, Reservation, Customer, InventoryTransaction
from .decorators import role_required

# ===============================================
# ✅ دالة الكول سنتر (عرض المنتجات + تنفيذ الحجوزات)
# ===============================================
from django.shortcuts import render, redirect
from django.contrib.auth.decorators import login_required
from asgiref.sync import async_to_sync
from channels.layers import get_channel_layer
from .models import Product, Branch, Inventory, Reservation, Category
...
def callcenter_dashboard(request):
    ...
    # ✅ حفظ الفئة المختارة في الـ Session
    ...
    if request.method == "POST":
        try:
            ...
            print("📩 تم استقبال POST من الكول سنتر")
            print(f"🧮 product={product.id}, branch={branch.id}, qty={qty}")
            print(f"📦 الكمية الحالية في المخزون: {inventory.quantity}")

            if inventory.quantity >= qty:
                print(f"✅ خصم {qty} من {product.name} - قبل: {inventory.quantity}")
                ...
                print(f"💾 بعد الحفظ: {inventory.quantity}")
                ...
                # ⏱️ تأخير بسيط لضمان وصول الإشارة قبل تحديث الصفحة (اختياري)
