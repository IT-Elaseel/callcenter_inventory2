...
def update_inventory(request):
    ...
    return render(
        ...
        {
            "categories": categories,
            "second_categories": second_categories,  # ✅ أضفها هنا
...
@login_required
@role_required(["branch"])
def update_inventory_stamp(request):
    profile = getattr(request.user, "userprofile", None)
    branch = profile.branch if profile else None

    if not branch:
        return render(request, "orders/no_permission.html",
                      {"error_message": "🚫 لا يوجد فرع مربوط بحسابك."}, status=403)

    if request.method == "POST":
        # 🟢 تحديث الكميات دفعة واحدة
        for key, value in request.POST.items():
            if key.startswith("qty_"):
                try:
                    product_id = int(key.replace("qty_", ""))
                    qty = int(value)
                    stamp = InventoryStamp.objects.get(branch=branch, product_id=product_id)
                    stamp.last_quantity = qty
                    stamp.save()

                    # 🔁 حدث المخزون الفعلي كمان
                    inventory, _ = Inventory.objects.get_or_create(branch=branch, product_id=product_id)
                    inventory.quantity = qty
                    inventory.save()
                except Exception as e:
                    print("❌ Error updating:", e)
        messages.success(request, "✅ تم تحديث جميع الكميات بنجاح.")
        return redirect("update_inventory_stamp")

    # 🟢 GET - عرض المنتجات الجاهزة من الاستامبا
    stamps = InventoryStamp.objects.filter(branch=branch).select_related("product__category")
    products = Product.objects.filter(is_available=True)
    categories = Category.objects.all()
    second_categories = SecondCategory.objects.all()

    return render(request, "orders/update_inventory_stamp.html", {
        "stamps": stamps,
        "products": products,
        "categories": categories,
...
def add_daily_request(request):
    ...
    if request.method == "POST":
        ...
        if "load_standard" in request.POST:
            standard_items = StandardRequest.objects.filter(branch=branch)
...
def set_standard_request(request):
    ...
    if request.method == "POST":
        if "add_item" in request.POST:
            ...
            if std_id and new_qty:
                try:
                    sr = StandardRequest.objects.get(id=std_id, branch=branch)
            ...
            std_id = request.POST.get("request_id")
            if std_id:
                StandardRequest.objects.filter(id=std_id, branch=branch).delete()
            return redirect("set_standard_request")
    ...
    categories = Category.objects.all()
    second_categories = SecondCategory.objects.all()
    standard_items = StandardRequest.objects.filter(branch=branch).select_related("product__category").order_by("product__category__name", "product__name")

    return render(request, "orders/set_standard_request.html", {
        "products": products,
        "categories": categories,
        "second_categories": second_categories,
        "requests_today": standard_items,  # نفس الاسم عشان HTML ما يتكسرش
        "selected_category": selected_category,
