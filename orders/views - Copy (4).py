...
def import_products(request):
    if request.method == "POST" and request.FILES.get("excel_file"):
        ...
        try:
            ...
            # ✅ الأعمدة المطلوبة (بدون unit وبدون description)
            expected_headers = [
            ...
            if any(h not in headers for h in expected_headers):
                messages.error(
                    request,
                    "❌ ملف Excel غير صحيح، يجب أن يحتوي على الأعمدة التالية بالضبط:\n"
                    "name, price, category_name, second_category_name"
                )
                return redirect("import_products")

            header_index = {h: headers.index(h) for h in expected_headers}
            count = 0

            for row in sheet.iter_rows(min_row=2, values_only=True):
                name = row[header_index["name"]]
                price = row[header_index["price"]]
                category_name = row[header_index["category_name"]]
                second_category_name = row[header_index["second_category_name"]]
                ...
                Product.objects.create(
                    name=str(name).strip(),
                    # description=str(description or "").strip(),  # ❌ مؤقتًا معطل
                    price=price_value,
                    category=category,
                    second_category=second_category,
                    unit="piece"  # 👈 ثابت دايمًا
                )
