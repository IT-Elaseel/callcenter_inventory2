...
def update_inventory(request):
    ...
    stamp_items = None  # 🔸 لتخزين بيانات الاستامبا عند التحميل

    # 🟢 POST = تحديث الكمية (يدوي أو استامبا)
    if request.method == "POST":

        # ✅ تحميل استامبا "تحديث المخزون" فقط
        if "load_stamp" in request.POST:
            stamp_items = StandardRequest.objects.filter(
                branch=branch,
                stamp_type="inventory"  # 👈 هنا الفرق الأساسي
            ).select_related("product")
            if not stamp_items.exists():
                messages.warning(request, "⚠️ لا توجد استامبا لتحديث المخزون لهذا الفرع.")
            else:
                messages.success(request, "✅ تم تحميل استامبا تحديث المخزون بنجاح.")

        # ✅ تحديث كل المنتجات من الاستامبا
            ...
            messages.success(request, f"✅ تم تحديث {updated} منتج من استامبا المخزون بنجاح.")
            return redirect("update_inventory")

        # ✅ التحديث الفردي (الكود الأصلي كما هو)
        else:
            product_id = request.POST.get("product_id")
            qty = request.POST.get("quantity")
    ...
    return render(
        ...
        "orders/update_inventory.html",
        {
            ...
            "inventories": inventories,
            "branch": branch,
            "stamp_items": stamp_items,  # 👈 لعرض الاستامبا عند التحميل
        },
    )
...
def set_inventory_stamp(request):
    ...
    categories = Category.objects.all()
    second_categories = SecondCategory.objects.all()
    inventory_stamps = StandardRequest.objects.filter(branch=branch, stamp_type="inventory").select_related("product__category").order_by("product__category__name", "product__name")
...
def set_standard_request(request):
    ...
    if request.method == "POST":
        if "add_item" in request.POST:
            ...
            if product_id and qty > 0:
                product = Product.objects.get(id=product_id)
                # ✅ تعديل: كل التعامل هنا يكون مع استامبا الطلبية فقط
                StandardRequest.objects.update_or_create(
                    branch=branch,
                    product=product,
                    stamp_type="order",  # ✅ النوع الصحيح
                    defaults={
                        "default_quantity": qty,
                        "updated_at": timezone.now()
                    }
    ...
    categories = Category.objects.all()
    second_categories = SecondCategory.objects.all()
    # ✅ عرض فقط استامبا الطلبية
    standard_items = StandardRequest.objects.filter(branch=branch, stamp_type="order").select_related("product__category").order_by("product__category__name", "product__name")
