...
def add_daily_request(request):
    ...
    # 🔑 order_number مؤقت مخزن في السيشن
    order_number = request.session.get("current_order_number")
    if not order_number:
        last_order = DailyRequest.objects.aggregate(Max("order_number"))["order_number__max"] or -1
    ...
    if request.method == "POST":
        if "add_item" in request.POST:
            ...
            now = timezone.now()
            DailyRequest.objects.filter(order_number=order_number, branch=branch).update(is_confirmed=True,confirmed_at=now)
    ...
    products = Product.objects.all()
    requests_today = DailyRequest.objects.filter(order_number=order_number, branch=branch, is_confirmed=False)
